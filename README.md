...
功能:
* `CMDOutput2Variable.bat` 将命令行输出保存到变量中。
* `DateTime.bat` 获取当前的日期时间。
* `FileCount.bat` 计数文件夹中文件。
* `FileOrFolder.bat` 判断当前对象是文件还是文件夹。
* `Iterate_loop.bat` 循环获取全部bat参数。
* `Pushd_Popd.bat` push pop功能。
* `read_file_line_by_line.bat` 一行一行的读取文件。
* `replace.bat` 变量字符串替换。
* `replace_all.bat` 批量字符串替换。
* `WhetherSubstring.bat` 是否是子串。
